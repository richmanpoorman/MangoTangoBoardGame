...
public class ComposableRules : Rules
{
    private int scoutWeight; 
    private bool scoutDivides;
    private bool offensiveScoutWeight;

    public ComposableRules(int _scoutWeight, bool _scoutDivides, bool _offensiveScoutWeight) {
        scoutWeight          = _scoutWeight; 
        scoutDivides         = _scoutDivides; 
    ...
    public bool isValidPush(Board board, Location from, Location to, Piece.Color playerTurn) {
        ...
        if (scoutDivides) { // If the scout is allowed to divide the 

            // If the push is in the "middle" of the current player's tiles
            ...
            if (board.isOnBoard(oppositeSideSquare) && board.tileAt(oppositeSideSquare).color() == playerTurn) {
                ...
                if (scoutOnDivider == null || scoutOnDivider.color() != playerTurn) return false; 
            }
        }

        // Then check for the weighting
        ...
        for (Vector2I position  = new Vector2I(from.row(), from.column()), 
            ...
            if (addingCurrentPlayer) tileSum += 1 + (currentTileHasScout ? scoutWeight : 0); 
            else tileSum -= 1 + (currentTileHasScout && !offensiveScoutWeight ? scoutWeight : 0); 
        }
